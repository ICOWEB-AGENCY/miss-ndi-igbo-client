...
export async function getServerSideProps({ req }) {
  try {
    ...
    const data = await getProtectedData("admin/users?order=1", {
      token,
      refreshToken,
    });
    console.log(data);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button title="Continue" onClick={clearValues} />
      </main>
    </div>
  );
};
  ...
  return (
    <>
      ...
      >
        ...
        {votedModalOpen && (
          <VotingSuccessModal setVisible={setVotedModalOpen} />
        )}
